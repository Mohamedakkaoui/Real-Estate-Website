...
exports.getListingByIdDB = async (id) => {
  try {
    const Listing = await ListingsSchema.findOne({ Object_id: id })
      .select(" -__v")
      .populate("owner", "FirstName LastName Email PhoneNumber ProfilePic _id");
    return Listing;
  } catch (error) {
...
exports.getAllListingDB = async () => {
  try {
    const listings = await ListingsSchema.find().populate(
      "owner",
...
exports.UpdateListingDB = async (id, data) => {
  try {
    return await ListingsSchema.findOneAndUpdate({ Object_id: id }, data, {
      new: true,
    });
  } catch (error) {
    return error;
...
exports.getMyListingsDB = async (UserId) => {
  try {
    if (mongoose.Types.ObjectId.isValid(UserId)) {
      const Listings = await ListingsSchema.find({ owner: UserId }).select(
        "title location images Object_id -_id"
      );
      return Listings;
    }
  } catch (error) {
