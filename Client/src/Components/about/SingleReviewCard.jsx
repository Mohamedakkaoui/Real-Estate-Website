...
const SingleReviewCard = ({review}) => {
  const { owner, rating , comment , date} = review
  const {ProfilePic, Username} = owner
  return (
      ...
      <div className="mt-6 flex items-center gap-6">
        <div className="h-10 w-10 overflow-hidden rounded-full shadow-sm outline-neutral-800">
          <div className="relative inline-block overflow-hidden rounded-lg border-neutral-800">
            <img alt={Username} src={ProfilePic} width="50" height="50" decoding="async" className="inline-block" loading="lazy" style={{ color: 'transparent' }} />
          </div>
        </div>
        <div>
          <p className="leading-relaxed tracking-wide text-gray-200">{Username}</p>
        </div>
      </div>
    </div>
  );
